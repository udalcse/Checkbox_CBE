	...
	public partial class MemcachedClient : IMemcachedClient, IMemcachedResultsClient
	{
		...
		protected virtual IStoreOperationResult PerformStore(StoreMode mode, string key, object value, uint expires, ref ulong cas, out int statusCode)
		{
			...
			if (node != null)
			{
                ...
                if (item.Data.Array.Length > 1000000)
                {
                    printPropertiesSizes(value.GetType().Name, value, new List<object>());
                }
                ...
                if (commandResult.Success)
                {
                    ...
                    result.Pass();
                    /*
                    if (item.Data.Array.Length > 1000)
                    {
                        System.Diagnostics.Trace.WriteLine(string.Format("Saved item of type {0}, {1} bytes", value.GetType().Name, item.Data.Array.Length));
                    }
                     */
                    return result;
                }
                ...
                {
                    System.Diagnostics.Trace.WriteLine(string.Format("Data size of failed item: {0}", item.Data.Array.Length));
                }
        ...
        private void printPropertiesSizes(string prefix, object value, List<object> processedObjects)
        {
            ...
            foreach (var f in fields)
            {
                try
                {
                    ...
                    if (fieldValue == null || processedObjects.Contains(value))
                    {
                        continue;
                    }
                    var item = getObjectDataLength(fieldValue);
                    //System.Diagnostics.Trace.WriteLine(string.Format("{0} = {1} bytes", prefix + "." + f.Name, item));
                    if (item > 1000)
                    {
                        processedObjects.Add(fieldValue);
                        printPropertiesSizes(prefix + "." + f.Name, fieldValue, processedObjects);
                    }
                }
                catch (Exception ex)
                {
            ...
            foreach (var f in props)
            {
                try
                {
                    var ggm = f.GetGetMethod();
                    if (ggm == null)
                        continue;
                    if (ggm.GetParameters().Length > 0)
                        continue;
                    var fieldValue = ggm.Invoke(value, new object[] { });
                    if (fieldValue == null || processedObjects.Contains(value))
                    {
                        continue;
                    }
        ...
        private long getObjectDataLength(object value)
        {
            var d = transcoder.Serialize(value);

            //MemoryStream ms = new MemoryStream();
            //BinaryFormatter bf = new BinaryFormatter();
            //bf.Serialize(ms, value);
            return d.Data.Array.Length;//ms.Length;
