<%@ Control Language="C#" CodeBehind="FileUploadAjaxifyed.ascx.cs" Inherits="CheckboxWeb.Forms.Surveys.Controls.ItemRenderers.FileUploadAjaxifyed" %>
...
<asp:Panel ID="_containerPanel" runat="server" CssClass="itemContainer">
    <script type="text/javascript">
        $(function() {
            $('#clearButton_<%=Model.ItemId %>').on('click', function () {
                var file = $('#<%=_uploadedFilePathTxt.ClientID %>').val();
                if (typeof file != "undefined" && file != '') {
                    var url = '<%=ResolveUrl("~/Upload.ashx") + "?op=delete&key=" + _uploader.UploadKey + "&random=" + Guid.NewGuid() %>&f=' + file;
                    $.ajax({
                        url: url,
                        type: 'POST',
                        cache: false,
                        ...
                        error: function () {
                            alert('Failed to delete the uploaded file');
                        },
                    });
                } else {
                    clear_<%=Model.ItemId %>();
            ...
            $('#<%= _inputPanel.ClientID %> input[type="file"]').on('file-upload-reset', function () {
                clear_<%=Model.ItemId %>();
            });
        });
        ...
        function onRespondentFileUploaded_<%=Model.ItemId %>(fileData) {
            if (fileData != null) {
                $('#<%=_uploadedFileNameTxt.ClientID %>').val(fileData.name);
                $('#<%=_uploadedFilePathTxt.ClientID %>').val(fileData.TempName);

                $('#<%= _uploadedFileName.ClientID %>').text(fileData.name);
        ...
        <asp:Panel ID="_contentPanel" runat="server" CssClass="itemContent">
            ...
            <asp:Panel ID="_bottomAndOrRightPanel" runat="server" CssClass="bottomAndOrRightContainer">
                ...
                <asp:Panel ID="_uploadedFilePanel" runat="server" style="display: none;">
                    ...
                    <a id="clearButton_<%=Model.ItemId %>" data-role="button" href="javascript:void();" class="ckbxButton roundedCorners border999 shadow999 orangeButton" ><%= TextManager.GetText("/controlText/UploadItemRenderer/clear") %></a>
                </asp:Panel>
                <asp:Panel ID="_inputPanel" runat="server" CssClass="inputContainer" style="display: none">
                    ...
                    <ckbx:Uploader ID="_uploader" SurveyMode="True" runat="server" Width="600" SelectFilePromptTextId="/controlText/fileUploader/selectFile" />
                    <div>
