    ...
    $.fn.listswap = function(options) {		
		...
		this.each(function () {
			...
			if($(this).attr('datatext')) {
				var text_data = '<p' + truncate_class + '>' + $(this).attr('datatext') + '</p>';
				$(div_id + ' .' + class_name).append(text_data);
			}
			
			if($(this).attr('datasearch')) {
				var search_data = '<div class="listbox_search">' + 
								  	'<input type="text" id="search_listbox" name="search_listbox" value="'+ $(this).attr('datasearch') + '" />' + 
			...
			if(!$(this).attr('datatext') && !$(this).attr('datasearch')){
			...
			(i % 2 !== 0) ? source_search = $(this).attr('datasearch') : destination_search = $(this).attr('datasearch');
		...
		function reset_search_input(search_selector, select_id) {
			$(search_selector + ' .listbox_search input#search_listbox').focus(function(){
				var val = $(this).val().toString().toLowerCase();
				var data_search = $(div_id + ' select#' + select_id).attr('datasearch').toString().toLowerCase();
				if(val == data_search)
					$(this).val('');
			});
			$(search_selector + ' .listbox_search input#search_listbox').blur(function(){
				var val = $(this).val().toString().toLowerCase();
				var data_search = $(div_id + ' select#' + select_id).attr('datasearch').toString();
				if(val == '')
