...
;(function($, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        this.options.hiddenInput = $("[id*=" + this.options.hiddenInput + "]");
    ...
    Plugin.prototype = {
        ...
        selectItem: function(that) {
            if (this.options.maxSelect == 0 || this.isBelowMax()) { //allow selection if there's no max or selection is below max
                ...
                this.addValueToHiddenInput($self.data('value'));
            }
        },
        ...
        removeItem: function(that) {
            ...
            this.removeValueFromHiddenInput($self.data('value'));
        },
        ...
        addValueToHiddenInput : function(value) {
            if (this.options.hiddenInput.length > 0) {
                var previousValue = this.options.hiddenInput.val().trim();
                var newValue = (previousValue + " " + value).trim();
                this.options.hiddenInput.val(newValue);
            }
        },

        removeValueFromHiddenInput : function(value) {
            if (this.options.hiddenInput.length > 0) {
                var previousValue = this.options.hiddenInput.val().trim();
                var newValue = previousValue.replace(value, '').trim();
                this.options.hiddenInput.val(newValue);
            }
        },

        clearHiddenInputValue : function() {
            if (this.options.hiddenInput.length > 0)
                this.options.hiddenInput.val('');
        },
        ...
        updateList: function() {
            ...
            this.$element.children().each(function() {
                ...
                if ($tmpThis.is(':selected')) {
                    ...
                    that.addValueToHiddenInput(tmpVal);
                }
