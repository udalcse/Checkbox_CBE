...
<%@ Control Language="C#" AutoEventWireup="false" CodeBehind="LineGraph.ascx.cs" Inherits="CheckboxWeb.Controls.Charts.LineGraph" %>
		    ...
		    $(function () {
		        ...
		        $(document).ready(function () {
                        ...
                        %> <% if (iSeries > 0) {%>,<%} %>
                            {
                                categories: [
                                    ...
                                    %> <% if (iCategory > 0) {%>,<%} %>
                                    '<%=Utilities.AdvancedHtmlEncode(EscapeJSStringConst(Wrap(xAxisCategories[iSeries][iCategory])))%>'
                                    <%}%>
                                ]
                            }
                        <%} %>                
                    ...
                    %>
                        ...
                        {
		                    type: 'line',
		                    name: htmlDecode('<%=EscapeJSStringConst(Series[s].Name)%>'),
                            ...
                            <% 
                                ...
                                <%=(hasAnswer ? "," : "") %> {name: htmlDecode('<%=Utilities.StripHtmlAndEncode(EscapeJSStringConst(Wrap(Series[s].AggregateResults[row].ResultText))) %>'), y : <%=UsePointsAsY ? Series[s].AggregateResults[row].Points : Series[s].AggregateResults[row].AnswerCount %>, percentage : <%=Series[s].AggregateResults[row].AnswerPercent.ToString().Replace(",", ".") %>} 
                                <% hasAnswer = true;} %>
                            <%}; %>
                            ]
		                }
