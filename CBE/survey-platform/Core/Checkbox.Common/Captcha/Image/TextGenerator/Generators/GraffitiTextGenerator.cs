...
namespace Checkbox.Common.Captcha.Image.TextGenerators
{
	...
	/// </summary>
	internal class GraffitiTextGenerator : ITextGenerator
	{
	    public void DrawText(string text, Bitmap textDestination)
		{
			...
			RectangleF rectInner=new RectangleF(rectImage.Width*0.01f,rectImage.Height*0.0f,rectImage.Width*0.95f,rectImage.Height*0.8f);

			//rectangle to fill with text
			RectangleF rectFBorders=rand.GetBetweenRectangle(rectImage,rectInner);
			
			int PixelCountConst=(int)(rectImage.Width*rectImage.Height*0.5f);
			...
			objCurvedText.DrawText(rectFBorders,g,font,text,fillBrush,pen);
