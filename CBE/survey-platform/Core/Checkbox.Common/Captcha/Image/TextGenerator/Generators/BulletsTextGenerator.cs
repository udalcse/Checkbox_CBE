...
namespace Checkbox.Common.Captcha.Image.TextGenerators
{
	...
	/// </summary>
	internal class BulletsTextGenerator : ITextGenerator
	{
	    public void DrawText(string text, Bitmap textDestination)
		{
			...
			Color lettersColor=Color.Red; 
			Color holeColor=backColor;
			//background 
			g.Clear(backColor);

			float fHoleCountConst=4f; //PARAM the higher value -> more holes on the letters
