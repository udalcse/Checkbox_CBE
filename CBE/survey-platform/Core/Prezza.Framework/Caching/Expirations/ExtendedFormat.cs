...
namespace Prezza.Framework.Caching.Expirations
{
	...
	internal class ExtendedFormat
	{
		...
		public ExtendedFormat(string format)
		{
			...
			{
				throw new Exception("Invalid extended format arguments.");
			}
		...
		private void ParseMinutes(string[] parsedFormat)
		{
			...
			foreach (int minute in this.minutes)
			{
				...
				{
					throw new ArgumentOutOfRangeException("format", "Specified minute value [" + minute.ToString() + "] was out of range.");
				}
			}
		}
		...
		private void ParseHours(string[] parsedFormat)
		{
			...
			foreach (int hour in this.hours)
			{
				if ((hour > 23) || (hour < -1))
				{
					throw new ArgumentOutOfRangeException("format", "Specified hour value [" + hour.ToString() + "] was out of range.");
				}
			}
		}
		...
		private void ParseDays(string[] parsedFormat)
		{
			...
			foreach (int day in this.days)
			{
				if ((day > 31) || (day < -1))
				{
					throw new ArgumentOutOfRangeException("format", "Specified day value [" + day.ToString() + "] was out of range.");
				}
			}
		}
		...
		private void ParseMonths(string[] parsedFormat)
		{
			...
			foreach (int month in this.months)
			{
				if ((month > 12) || (month < -1))
				{
					throw new ArgumentOutOfRangeException("format", "Specified month [" + month.ToString() + "] was out of range.");
				}
			}
		}
		...
		private void ParseDaysOfWeek(string[] parsedFormat)
		{
			...
			foreach (int dayOfWeek in this.daysOfWeek)
			{
				...
				{
					throw new ArgumentOutOfRangeException("format", "Specified day of week [" + dayOfWeek.ToString() + "] was out of range.");
				}
			}
		}
